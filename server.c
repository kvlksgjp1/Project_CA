...
void init()
{
	position.row=1;
	position.col=1;
	position.power = 3; //물풍선 세기 초기 3
	position.bomb_num = 1; //물풍선 갯수 초기 1 
}
...
void explosion(int row,int col){
	...
	update(row,col,'B');
	for(i=1;i<=power;i++){
		if(map[row][col+i]!='2' && map[row][col+i] != '3' && map[row][col+i] != '4' && map[row][col+i] !='1')
		{
			if(map[row][col+i]=='M')
			{
				explosion(row,col+i);
	...
	for(i=1;i<=power;i++){
		if(map[row+i][col]!='2'  && map[row+i][col] != '3' && map[row+i][col] != '4' && map[row+i][col] != '1')
		{
			if(map[row+i][col]=='M')
			{
				explosion(row+i,col);
...
void random_item()
{
	...
	for (i = 0; i < 20; i++) {
		...
		temp = rand() % 2;		   //0이면 물풍선 세기, 1이면 물풍선 갯수 

		if (map[row][col] == 1) {
			if (temp == 0)
			{
				map[row][col] = 3; // 물풍선 길이 아이템이 있는 벽
			}
			else {
				map[row][col] = 4; // 물풍선 갯수 아이템이 있는 벽
			}
		}
	}	
}