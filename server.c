...
int	delay = 50;	/* how long to wait	*/
int	done  = 0;
int power = 1;

player_information position;
...
void init()
{
	...
	position.row=1;
	position.col=1;
	position.bomb_num_max=1;
	position.bomb_available=1;
	position.power=3;
	position.isAlive=1;
}
...
if(contain(chatData,"LEFT")&&position.isAlive==1)
         {
            if(map[position.row][position.col-1]!='2' && map[position.row][position.col-1]!='M' && map[position.row][position.col-1]!='1'){
               if(map[position.row][position.col]!='M' && map[position.row][position.col]!='1'){
                  if (map[position.row][position.col - 1] == '3') {
		     if(position.power<=MAX_POWER_LEN)
                     {
			position.power++;
                	power++;
		     }                     
                  }
                  else if (map[position.row][position.col - 1] == '4') {
			if(position.bomb_num_max<=MAX_BOMB_NUM)
                     		{position.bomb_num_max++;}                   
                  }
         ...
         else if(contain(chatData,"RIGHT")&&position.isAlive==1)
         {
            if(map[position.row][position.col+1]!='2' && map[position.row][position.col+1]!='M' && map[position.row][position.col+1]!='1'){
               if(map[position.row][position.col]!='M' && map[position.row][position.col]!='1'){
                  if (map[position.row][position.col + 1] == '3') {
         ...
         else if(contain(chatData,"UP")&&position.isAlive==1)
         {
			...
			position.power++;
                  ...
                  else if (map[position.row-1,position.col] == '4') {
			if(position.bomb_num_max<=MAX_BOMB_NUM)
                     		{position.bomb_num_max++;} 
         ...
         else if(contain(chatData,"DOWN")&&position.isAlive==1)
         {printf("%d %d\n",position.bomb_available,position.bomb_num_max);
            if(map[position.row+1][position.col]!='2' && map[position.row+1][position.col]!='M' && map[position.row+1][position.col]!='1'){
               if(map[position.row][position.col]!='M' && map[position.row][position.col]!='1'){
                  if (map[position.row+1][position.col] == '3') {
			if(position.power<=MAX_POWER_LEN)
         ...
         else if(contain(chatData,"BOMB")&&position.isAlive==1)
         {printf("%d %d\n",position.bomb_available,position.bomb_num_max);
             if(map[position.row][position.col]!='M'){
		printf("%d %d\n",position.bomb_available,position.bomb_num_max);
		if(position.bomb_available<=position.bomb_num_max){
			position.bomb_available++;
			printf("%d %d\n",position.bomb_available,position.bomb_num_max);
         ...
         else if(contain(chatData,"WALL")&&position.isAlive==1)
         {
...
void *boom() /*2018.11.30 KJY add*/
{
	...
	p = (player_information *)malloc(sizeof(player_information));
	p->row=position.row;
	p->col=position.col;
	...
	if(map[p->row][p->col]=='M')/* if it doesn't explode with earlier other thread*/
		thr_id=pthread_create(&thr,NULL,explosion,(void*)p);
}

void *explosion(void *arg){
	int temp;
	int i=0,row,col;
	char *ptr;
	int thr_id;
	pthread_t thr;
	player_information *bomb = (player_information *)arg;
	player_information *p;
	p = (player_information *)malloc(sizeof(player_information));
	row=bomb->row;
	col=bomb->col;	
			...
			if(map[row][col-i]=='M')
				{
					p->row=row;
					p->col=col-i;
					pthread_create(&thr, NULL, explosion, (void*)p);
					break;
				}	
			update(row,col-i,'B');
		}
